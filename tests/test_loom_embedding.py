"""Tests for xera.xl.embedding: Embedding, RotaryEmbedding."""
...
def test_embedding_and_rotary():
    emb = xl.Embedding(num_embeddings=100, features=16, key=jax.random.PRNGKey(0))
    idx = jnp.array([[1, 2, 3], [4, 5, 6]])
    out = emb(idx)
    assert out.shape == (2, 3, 16)

    rope = xl.RotaryEmbedding(dim=16)
...
def test_embedding_lookup_matches_table_row():
    emb = xl.Embedding(num_embeddings=10, features=4, key=jax.random.PRNGKey(0))
    idx = jnp.array([[3]])
    out = emb(idx)
