"""Tests for xera.loom.recurrent: SSM, SelectiveSSM, MambaBlock (SSM family)."""
...
def test_ssm_dt_within_bounds():
    ssm = loom.SSM(channels=16, state_dim=4, dt_min=0.001, dt_max=0.1, key=jax.random.PRNGKey(0))
...
def test_ssm_output_depends_on_full_history_at_last_step():
    ssm = loom.SSM(channels=4, state_dim=4, key=jax.random.PRNGKey(0))
...
def test_selective_ssm_forward_shape():
    ssm = loom.SelectiveSSM(d_inner=16, state_dim=4, key=jax.random.PRNGKey(0))
...
def test_selective_ssm_default_dt_rank():
    ssm = loom.SelectiveSSM(d_inner=32, state_dim=4, key=jax.random.PRNGKey(0))
    assert ssm._dt_rank == max(1, 32 // 16)


def test_selective_ssm_custom_dt_rank():
    ssm = loom.SelectiveSSM(d_inner=32, state_dim=4, dt_rank=3, key=jax.random.PRNGKey(0))
...
def test_selective_ssm_causal_future_does_not_affect_past():
    ssm = loom.SelectiveSSM(d_inner=8, state_dim=4, key=jax.random.PRNGKey(0))
...
def test_mamba_block_forward_shape():
    mamba = loom.MambaBlock(d_model=16, state_dim=4, key=jax.random.PRNGKey(0))
    x = jax.random.normal(jax.random.PRNGKey(1), (2, 10, 16))
    out = mamba(x)
    assert out.shape == (2, 10, 16)
...
def test_mamba_block_default_inner_dim():
    mamba = loom.MambaBlock(d_model=16, key=jax.random.PRNGKey(0))
    assert mamba._d_inner == 32


def test_mamba_block_custom_inner_dim():
    mamba = loom.MambaBlock(d_model=16, d_inner=48, key=jax.random.PRNGKey(0))
...
def test_mamba_block_has_submodules():
    mamba = loom.MambaBlock(d_model=16, key=jax.random.PRNGKey(0))
    assert isinstance(mamba.conv, loom.Conv)
    assert isinstance(mamba.ssm, loom.SelectiveSSM)
    assert isinstance(mamba.in_proj, loom.Dense)
    assert isinstance(mamba.out_proj, loom.Dense)


def test_mamba_block_causal_conv_future_does_not_affect_past():
    mamba = loom.MambaBlock(d_model=8, state_dim=4, conv_kernel=4, key=jax.random.PRNGKey(0))
...
def test_mamba_block_grad_shapes_match_params():
    mamba = loom.MambaBlock(d_model=8, state_dim=4, key=jax.random.PRNGKey(0))
    x = jax.random.normal(jax.random.PRNGKey(1), (2, 5, 8))
...
def test_mamba_block_stack_composability():
    ...
    blocks = [loom.MambaBlock(d_model=8, state_dim=4, key=k) for k in keys]
    x = jax.random.normal(jax.random.PRNGKey(1), (1, 6, 8))
    for block in blocks:
        x = block(x)
