...
the portable `xenafl_attention` fallback.
...
TODO:
    ...
    - `supports_cudnn_fused_attention(device) -> bool` -- True iff
      ...
      *in addition to* the existing dtype check, and route to
      `xenafl_attention` when the device is pre-Ampere specifically.

Important: this capability check only ever affects the default routing
choice made when `backend` is left as `None` (auto-dispatch).
`xenafl_attention` is a correct, portable kernel that works (just not
at vendor-kernel speed) on any device/GPU arch when explicitly
requested via `backend="xenafl"` -- this module's checks must never
gate or reject an explicitly-forced backend, only influence what
auto-dispatch picks by default.
